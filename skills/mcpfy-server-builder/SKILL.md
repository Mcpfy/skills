...
- A widget that fetches an external API or loads external images/fonts needs `widget: { dir, csp: { connectDomains, resourceDomains } }` (the object form, not the bare string) — without it those calls are fine by default, but adding `csp` at all makes it a strict allowlist, and it also blocks `data:` image URIs. See [Widgets](references/widgets.md).
